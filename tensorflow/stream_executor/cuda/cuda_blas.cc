...
port::StatusOr<cublasLtEpilogue_t> CUBLASEpilogue(blas::Epilogue epilogue) {
  switch (epilogue) {
    ...
    case blas::Epilogue::kBiasThenGeLUApproximate:
...
port::StatusOr<UniqueOpDesc> CreateCublasLtOperationDesc(
  ...
  TF_ASSIGN_OR_RETURN(cublasLtEpilogue_t epilog_op, CUBLASEpilogue(epilogue));
...
bool CUDABlas::DoBlasLtMatmulInternal(
  ...
  if ((cuda_plan.params().epilogue == blas::Epilogue::kBias ||
       ...
       cuda_plan.params().epilogue ==
           blas::Epilogue::kBiasThenGeLUApproximate) != (bias != nullptr)) {
    VLOG(2) << "DoBlasLtMatmul returning false because plan has wrong "
